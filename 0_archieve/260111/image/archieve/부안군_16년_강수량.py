...
csv_file = "/home/user/문서/workspace/python/graph/data/부안군_16년.csv"
...
fig, ax = plt.subplots()
bars = []
for i, (xi, yi, date) in enumerate(zip(idx, y, df["시점"])):
    # 🔥 6,7,8,9월이면 다른 색
    if date.month in [6, 7, 8, 9]:
        bar_color = colors[2]      # ← 강조할 색
    else:
        bar_color = colors[0]      # ← 기본 막대 색
    bars.append(
        ax.bar(xi, yi, color=bar_color, width=0.7, linewidth = 0.1,edgecolor="#444", zorder=3)
    )
